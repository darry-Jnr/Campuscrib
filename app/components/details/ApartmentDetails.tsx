...
const ApartmentDetail = ({
  ...
  const handleClick = () => {
    ...
    if (!token) {
      router.push(`/auth/login?redirect=${encodeURIComponent(window.location.pathname)}`);//redirect to login
    }
    else {
      router.push("/chats")
    }
  ...
  return (
    ...
    <BackButton />
      ...
      <div className="fixed bottom-0 left-0 w-full bg-white border-t border-gray-200 flex justify-between items-center p-4 md:p-6 shadow-md">
        ...
        <button onClick={handleClick} className="bg-green-500 text-white px-4 md:px-6 py-2 rounded-lg text-sm md:text-base font-medium hover:opacity-50 transition">
          Contact Agent
        </button>
      </div>
    </div>
